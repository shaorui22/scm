 ...
 * @date 2021/12/21
 */
public interface ScmUserDetailsService extends UserDetailsService, Ordered {
	...
	default UserDetails getUserDetails(R<UserInfo> result) {
		...
		// 构造security用户
		return new ScmUser(user.getUserId(), user.getDeptId(), user.getUsername(),
				SecurityConstants.BCRYPT + user.getPassword(), user.getPhone(), user.getEmail(),
				true, true, true,
	 ...
	 * @return
	 */
	default UserDetails loadUserByUser(ScmUser scmUser) {
		return this.loadUserByUsername(scmUser.getUsername());
	}
