...
package com.kava.scm.common.xss.core;

import cn.hutool.core.util.CharsetUtil;
import com.kava.scm.common.xss.config.ScmXssProperties;
import com.kava.scm.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final ScmXssProperties properties;

	public DefaultXssCleaner(ScmXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(ScmXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		ScmXssProperties.Mode mode = properties.getMode();
		if (ScmXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (ScmXssProperties.Mode.validate == mode) {
			// 校验
