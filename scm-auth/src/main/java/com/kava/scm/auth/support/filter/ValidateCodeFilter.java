package com.kava.scm.auth.support.filter;
...
import com.kava.scm.common.core.exception.ValidateCodeException;
import com.kava.scm.common.core.util.SpringContextHolder;
import com.kava.scm.common.core.util.WebUtils;
...
public class ValidateCodeFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		System.out.println("ValidateCodeFilter doFilterInternal .........");
		...
		// 校验验证码 1. 客户端开启验证码 2. 短信模式, 3. 邮箱模式
		try {
			checkCode(grantType);
			filterChain.doFilter(request, response);
		}
	 ...
	 * 校验验证码
	 */
	private void checkCode(String grantType) throws ValidateCodeException {
		...
		// https://gitee.com/log4j/csp/issues/IWA0D
		String key = null;
		if (SecurityConstants.MOBILE.equals(grantType)) {
			String mobile = request.get().getParameter(SecurityConstants.MOBILE);
			if (StrUtil.isNotBlank(mobile)) {
				key = CacheConstants.DEFAULT_CODE_KEY + mobile;
			}
		} else if (SecurityConstants.EMAIL.equals(grantType)) {
			String email = request.get().getParameter(SecurityConstants.EMAIL);
			if (StrUtil.isNotBlank(email)) {
				key = CacheConstants.DEFAULT_EMAIL_CODE_KEY + email;
			}
		} else if (SecurityConstants.PASSWORD.equals(grantType)) {
			String randomStr = request.get().getParameter(SecurityConstants.RANDOMSTR);
